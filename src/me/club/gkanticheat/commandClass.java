			...
			case "check_player":
				if (sender.hasPermission("gkac.check")) {
					if (args.length > 1) {
						...
						player.sendMessage(Main.systemName + ChatColor.WHITE + "   " + Main.whiteString + ": "
								+ ChatColor.YELLOW + white);
						player.sendMessage(
								Main.systemName + "   " + Main.failsString + ": " + ChatColor.YELLOW + fails);
						player.sendMessage(
								Main.systemName + "   " + Main.hacksString + ": " + ChatColor.YELLOW + hacks);
